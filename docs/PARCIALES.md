...
| Mono | Monotonicidad de composición D1-D4 (§7.4) | ✅ RESUELTO (Fase 4) | Implementado: `composition::compose` compone las capas por id y verifica D1–D4 contra el ancestro `locked`; violación = error con dimensión + capa. `merge:append` y `overridable` incluidos | `composition.rs`; `compile.rs::compile_layered` | STATUS inv15/7.4 ✅ |
