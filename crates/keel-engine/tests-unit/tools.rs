...
//! Unit tests for `tools` (relocated out of src; included via #[path] in src/tools.rs).
    ...
    fn ctx() -> ExecContext<'static> {
        ExecContext { workspace_root: Path::new(".") }
    }
    ...
    fn detector_contains_hits_and_misses() {
        let call = builtin_call("text.contains", serde_json::json!({"value": ".notifier.data"}));
        assert!(run_detector(&call, &event_with(Some("x.notifier.data"), None)));
    ...
    fn detector_command_classify_matches_families_and_globs() {
        ...
        assert!(run_detector(&call, &event_with(None, Some("psql -c 'SELECT 1'"))));
        assert!(run_detector(&call, &event_with(None, Some("php/artisan db:wipe --force"))));
    ...
    fn validate_builtin_match_is_invalid_absent_content_is_unknown() {
        ...
        assert_eq!(
            run_validate(&call, &event_with(Some("x // TODO"), None), &Default::default(), ctx()).verdict,
            Verdict::Invalid
        );
        assert_eq!(
            run_validate(&call, &event_with(Some("done"), None), &Default::default(), ctx()).verdict,
            Verdict::Valid
        );
        assert_eq!(
    ...
    fn precondition_env_and_flag_read_the_event_not_the_process() {
        ...
        assert!(!run_precondition(&env_pre, &bare, &Default::default(), ctx()));
    ...
    fn exit_code_tool_maps_three_states() {
        ...
        assert_eq!(run_external(&mk("0"), &event_with(None, None), ctx()).verdict, Verdict::Valid);
        assert_eq!(run_external(&mk("1"), &event_with(None, None), ctx()).verdict, Verdict::Invalid);
        assert_eq!(run_external(&mk("2"), &event_with(None, None), ctx()).verdict, Verdict::Unknown);
    }
    ...
    fn sarif_output_empty_results_is_valid() {
        ...
        assert_eq!(run_external(&def, &event_with(None, None), ctx()).verdict, Verdict::Valid);
    }
