...
mod tests {
    ...
    fn hash_ignores_created_at_and_is_stable() {
        let a = Snapshot::build(vec![rule("r1")], BTreeMap::new(), BTreeMap::new(), "2026-01-01T00:00:00Z".into())
            .unwrap();
        let b = Snapshot::build(vec![rule("r1")], BTreeMap::new(), BTreeMap::new(), "2026-06-01T12:00:00Z".into())
            .unwrap();
        assert_eq!(a.hash, b.hash);

        let c = Snapshot::build(vec![rule("r2")], BTreeMap::new(), BTreeMap::new(), "2026-01-01T00:00:00Z".into())
            .unwrap();
    ...
    fn tampered_snapshot_fails_to_load() {
        ...
        let snap =
            Snapshot::build(vec![rule("r1")], BTreeMap::new(), BTreeMap::new(), "2026-01-01T00:00:00Z".into())
                .unwrap();
