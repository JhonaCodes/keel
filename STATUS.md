...
| **inv 13** delegation limits (depth/time/cost/permissions) | 🟡 | timeout + maxTokens enforced (over budget → unknown + finding, real tokens in ledger); maxDepth/cross-cost deferred (need delegation graph, Phase 2); permissions (executor sandbox) = PROGRAMA T3 |
